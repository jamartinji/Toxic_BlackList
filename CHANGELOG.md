...
- **Targeted-by alert:** Lua error `sanitizeApiString` (nil) when scanning units that might have you targeted; the sanitizer is now declared before use so `ScanBlacklistedTargetingPlayer` resolves the correct local function.
